...
define(['jquery', 'util/template/2.0'], function($, Template) {
  ...
  function joinRightActionBar() {
    ...
    var noticeContent = noticeConfig.noticeName || '暂无公告';
    var closeBtn = '';
    var bulltinUrl = noticeConfig.url ? noticeConfig.url : 'javascript:void(0)';

    if (isUserFirstTimeVisit()) {
      closeBtn = '<div class="close-btn">关闭</div>';
    }

    return '<a href="'+bulltinUrl+'" target="_blank" class="call-board tips-closed"><i class="iconfont icon-alarm"></i>平台升级操作指南<div class="tips"><div class="content">'+noticeContent+'</div>'+closeBtn+'</div></a>';
  }
  ...
  function isUserFirstTimeVisit() {
    return !localStorage.getItem('tipLoaded'); // 要考虑当前有没有内容 再弹出
  }

  function setUserVisited() {
    localStorage.setItem('tipLoaded', 'loaded');
  }
  // 初始化公告面板
  // url:  公告btn, content 需要 点击
  // 初次进入 强制显示
  function bindBulletinTip() {
    var noticeConfig = window.NOTICE_CONFIG || {};
    var bulltinBoard = $('.call-board');
    var closeBtn = $('.close-btn');

    if (isUserFirstTimeVisit()) {
      // 展开 tip 面板
      bulltinBoard.removeClass('tips-closed');
    }
    ...
    bulltinBoard.hover(openTip, function() {
      if (!isUserFirstTimeVisit()) {
        closeTip();
      }
    });
    ...
    function closeTip() {
     bulltinBoard.addClass('tips-closed');
    }
  }
  ...
  var renderSideBar = function() {
    ...
    var curLeve1Node = window.PAGE_NAV_DATA.filter(function(level1Node) {
